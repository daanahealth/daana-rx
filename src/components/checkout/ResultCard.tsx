      ...
      <CardContent className="pt-6 pb-5 space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-sm text-muted-foreground">
              ...
              {quantity !== undefined && quantity !== null && <span>Qty: {String(quantity)}</span>}
            </div>
          </div>
        ...
        {isExpired && (
          <p className="text-sm text-destructive flex items-start gap-2">
            ...
            <span>Expired medication. Superadmin override required with a mandatory note.</span>
          </p>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-1">
          ...
          <Link
            href={`/inventory?unitId=${encodeURIComponent(item.id)}`}
            className="w-full sm:w-auto"
          >
