...
const CLINIC = { clinicId: 'c1', name: 'MASS Clinic', createdAt: '2026-01-01', updatedAt: '2026-01-01' };
const PROVIDER = { userId: 'u1', username: 'kpatel', email: 'karol.patel@massclinic.org', clinicId: 'c1', activeClinicId: 'c1', userRole: 'provider' };
const SUPER = { userId: 'u2', username: 'kim', email: 'kim@massclinic.org', clinicId: 'c1', activeClinicId: 'c1', userRole: 'superadmin' };
const PROFILE = { fullName: 'Karol Patel', credential: 'NP', specialty: 'CARDIO', active: true };

const card = (key: string, medicationName: string, dose: string, form: string, availableUnits: number, earliestExpiry: string | null, specialtyClass = 'CARDIO') => ({
  key, medicationName, dose, form, specialtyClass, availableUnits, availableQuantity: availableUnits, earliestExpiry,
});
const MEDS = [
...
const REQUESTS = [
  { id: 'r1', status: 'pending', medicationName: 'Lisinopril', dose: '10 mg', form: 'Tablet', quantity: 1, patientRef: '10423', denialReason: null, createdAt: iso(-12), expiresAt: iso(150), resolvedAt: null,
    provider: { userId: 'u1', ...PROFILE }, ageSeconds: 720, units: [{ itemId: 'i1', unitCode: 'DRX-MASS-CARDIO1-00042', locationCode: 'CARDIO1', expiryDate: '2027-03-07', status: 'pending_approval', released: false }] },
  { id: 'r2', status: 'pending', medicationName: 'Atorvastatin', dose: '20 mg', form: 'Tablet', quantity: 2, patientRef: null, denialReason: null, createdAt: iso(-4), expiresAt: iso(25), resolvedAt: null,
    provider: { userId: 'u3', fullName: 'Sam Ortiz', credential: 'MD', specialty: 'CARDIO' }, ageSeconds: 240, units: [{ itemId: 'i2', unitCode: 'DRX-MASS-CARDIO2-00007', locationCode: 'CARDIO2', expiryDate: '2026-09-10', status: 'pending_approval', released: false }, { itemId: 'i3', unitCode: 'DRX-MASS-CARDIO2-00008', locationCode: 'CARDIO2', expiryDate: '2026-09-10', status: 'pending_approval', released: false }] },
  { id: 'r3', status: 'fulfilled', medicationName: 'Amlodipine', dose: '5 mg', form: 'Tablet', quantity: 1, patientRef: '10311', denialReason: null, createdAt: iso(-90), expiresAt: iso(60), resolvedAt: iso(-80),
    provider: { userId: 'u1', ...PROFILE }, ageSeconds: 5400, units: [{ itemId: 'i4', unitCode: 'DRX-MASS-CARDIO1-00011', locationCode: 'CARDIO1', expiryDate: '2026-12-01', status: 'checked_out', released: true }] },
  { id: 'r4', status: 'denied', medicationName: 'Losartan', dose: '50 mg', form: 'Tablet', quantity: 1, patientRef: null, denialReason: 'could not locate', createdAt: iso(-200), expiresAt: iso(-10), resolvedAt: iso(-190),
    provider: { userId: 'u1', ...PROFILE }, ageSeconds: 12000, units: [] },
];
const mine = REQUESTS.filter((r) => r.provider.userId === 'u1').map(({ provider: _p, units: _u, ageSeconds: _a, ...r }) => r);

async function seed(page: Page, user: typeof PROVIDER) {
  await page.addInitScript(({ user, clinic }) => {
...
async function mockGateway(page: Page, flags: Record<string, unknown>, opts: { conflict?: boolean } = {}) {
  await page.route(`${GATEWAY}/**`, async (route) => {
    ...
    if (p === '/inventory/provider/home') return json({ specialty: 'CARDIO', available: MEDS.filter((m) => m.specialtyClass === 'CARDIO'), topRequested: [] });
    if (p === '/inventory/provider/medications') {
      ...
      if (q) rows = MEDS.filter((m) => m.medicationName.toLowerCase().includes(q)).concat(q.startsWith('war') ? [card('warfarin', 'Warfarin', '5 mg', 'Tablet', 0, null)] : []);
    ...
    if (detail) {
      ...
      return json({ medication: { ...m, nextExpiries: [{ expiryDate: m.earliestExpiry, availableUnits: 1, availableQuantity: 1 }, { expiryDate: '2027-08-01', availableUnits: m.availableUnits - 1, availableQuantity: m.availableUnits - 1 }] } });
    }
    if (p === '/transactions/requests/top') return json({ days: 30, topRequested: [{ medicationKey: 'lisinopril', medicationName: 'Lisinopril', dose: '10 mg', form: 'Tablet', count: 14 }, { medicationKey: 'atorvastatin', medicationName: 'Atorvastatin', dose: '20 mg', form: 'Tablet', count: 9 }] });
    ...
    if (p === '/transactions/requests' && method === 'GET') {
      ...
      const rows = REQUESTS.filter((r) => (status === 'pending' ? r.status === 'pending' : r.status !== 'pending'));
      return json({ status, requests: rows, count: rows.length });
    }
    if (p === '/transactions/requests' && method === 'POST') {
      if (opts.conflict) return json({ error: 'That unit was just reserved. Next available: Lisinopril, next FEFO expiry 2027-08-01', conflict: 'concurrent_reservation' }, 409);
      const body = route.request().postDataJSON();
      return json({ request: { id: 'r9', status: 'pending', medicationName: 'Lisinopril', dose: '10 mg', form: 'Tablet', quantity: body.quantity, patientRef: body.patientRef ?? null, denialReason: null, createdAt: iso(0), expiresAt: iso(180), resolvedAt: null } }, 201);
    }
    if (/^\/transactions\/requests\/[^/]+\/(fulfill|deny|return|cancel)$/.test(p)) {
      const id = p.split('/')[3];
      const r = REQUESTS.find((x) => x.id === id)!;
      const action = p.split('/')[4];
      const status = action === 'fulfill' ? 'fulfilled' : action === 'deny' ? 'denied' : action === 'cancel' ? 'cancelled' : r.status;
      return json({ request: { ...r, status, resolvedAt: iso(0), denialReason: action === 'deny' ? route.request().postDataJSON()?.reason : null } });
    }
    if (p.startsWith('/notifications')) return json({ notifications: [], unread: 0 });
    if (p === '/inventory/stats') return json({ totalUnits: 595, unitsExpiringSoon: 12, recentCheckIns: 30, recentCheckOuts: 18, lowStockAlerts: 2 });
    return json({ error: `unmocked ${method} ${p}` }, 404);
  });
}

const FLAGS_ON = { provider_requests_enabled: true, patient_ref_enabled: true, attestation_mode: 'none', request_ttl: 'end_of_day' };
const FLAGS_OFF = { ...FLAGS_ON, provider_requests_enabled: false };

test.beforeAll(() => fs.mkdirSync(SHOT, { recursive: true }));

test('provider: home, search, request modal, conflict, confirmation, inventory, my requests', async ({ page }) => {
  ...
  await expect(page.getByText('Denied: could not locate').locator('visible=true').first()).toBeVisible();
...
test('superadmin: request queue with badge, deny needs a reason, settings panel', async ({ page }) => {
