...
function record(name, method, path, res, opts = {}) {
  ...
  if (res.err) verdict = 'ERR';
    ...
    verdict = res.status >= 200 && res.status < 400 ? 'PASS' : res.status < 500 ? 'WARN' : 'FAIL';
  const note = res.err
    ? res.err
...
async function main() {
  ...
  if (r.data && r.data.token) {
    token = r.data.token;
    clinicId = r.data.clinic?.clinicId;
  }
  if (!token) {
    log('FATAL: signup did not return a token; cannot continue authed tests.');
    printMatrix();
    process.exit(1);
  }
  ...
  record(
    ...
    await call('GET', '/auth/check-email?email=' + encodeURIComponent(email)),
    { expectOk: true }
  );
  record(
    'auth requires token',
    'GET',
    '/auth/me (no token)',
    await call('GET', '/auth/me', { auth: false }),
    { expectStatus: 401 }
  );

  // ---- INVENTORY reads ----
  record('stats', 'GET', '/inventory/stats', await call('GET', '/inventory/stats'), {
    expectOk: true,
  });
  record('locations', 'GET', '/inventory/locations', await call('GET', '/inventory/locations'), {
    expectOk: true,
  });
  record(
    'locations/v2',
    'GET',
    '/inventory/locations/v2',
    await call('GET', '/inventory/locations/v2'),
    { expectOk: true }
  );
  record(
    'drugs/search',
    'GET',
    '/inventory/drugs/search?q=amoxicillin',
    await call('GET', '/inventory/drugs/search?q=amoxicillin'),
    { expectOk: true }
  );
  record('items list', 'GET', '/inventory/items', await call('GET', '/inventory/items'), {
    expectOk: true,
  });
  record(
    'next-code',
    'GET',
    '/inventory/items/next-code?location=E2E',
    await call('GET', '/inventory/items/next-code?location=E2E'),
    {}
  );
  record(
    'classification GET',
    ...
    await call('GET', '/inventory/settings/classification'),
    { expectStatus: 200 }
  );
  record(
    'expiry/report',
    ...
    await call('GET', '/inventory/expiry/report'),
    { expectOk: true }
  );
  ...
  r = record(
    ...
    'POST',
    '/inventory/locations',
    await call('POST', '/inventory/locations', { body: { name: locCode, temp: 'room temp' } }),
    { expectOk: true }
  );
  let legacyLocId =
    r.data?.location?.id || r.data?.location?.location_id || r.data?.location_id || r.data?.id;
  if (!legacyLocId) {
  ...
  record(
    'next-code (real location)',
    'GET',
    `/inventory/items/next-code?location=${locCode}`,
    await call('GET', `/inventory/items/next-code?location=${encodeURIComponent(locCode)}`),
    { expectStatus: 200 }
  );
  ...
  const attributes = {
    ...
    unit: 'mg',
    form: 'Bottle',
    specialty_class: 'INFECT',
  };
  r = record(
    'check-in (platform body: type_id+location_id)',
    'POST',
    '/inventory/items',
    await call('POST', '/inventory/items', {
      body: {
        type_id: MASS_TYPE_ID,
        location_id: legacyLocId,
        attributes,
  ...
  if (newItemId) {
    ...
    rows.push({
      name: 'check-in PERSISTS (item in list)',
      method: 'GET',
      path: '/inventory/items',
      status: list.status,
      verdict: found ? 'PASS' : 'FAIL',
      note: found ? 'item visible' : 'item NOT in list',
    });
  }
  ...
  r = record(
    'check-in (FE shape: typeName+locationCode)',
    'POST',
    '/inventory/items',
    await call('POST', '/inventory/items', {
      body: {
  ...
  if (feItemId) {
    ...
    rows.push({
      name: 'FE-shape check-in PERSISTS',
      method: 'GET',
      path: '/inventory/items',
      status: list.status,
      verdict: found ? 'PASS' : 'FAIL',
      note: found ? 'item visible in inventory' : 'item NOT in list',
    });
  }
  ...
  const customGuide = [
    {
      class_name: 'E2ETEST',
      common_examples: ['ZZZDrug'],
      location_code: 'E2EBIN',
      two_digit_code: 'ZZ',
  ...
  record(
    ...
    'PATCH',
    '/inventory/settings/classification',
    await call('PATCH', '/inventory/settings/classification', { body: { entries: customGuide } }),
    { expectStatus: 200 }
  );
  {
    ...
    rows.push({
      name: 'classification PERSISTS',
      method: 'GET',
      path: '/inventory/settings/classification',
      status: g.status,
      verdict: persisted ? 'PASS' : 'FAIL',
      note: persisted ? 'override saved' : 'override NOT saved',
    });
  }
  ...
  r = record(
    ...
    'POST',
    '/transactions/carts',
    await call('POST', '/transactions/carts'),
    { expectOk: true }
  );
  ...
  if (cartId && newItemId) {
    record(
      'add to cart',
      'POST',
      `/transactions/carts/${cartId}/items`,
      await call('POST', `/transactions/carts/${cartId}/items`, { body: { item_id: newItemId } }),
      {}
    );
    record(
      'approve cart',
      'POST',
      `/transactions/carts/${cartId}/approve`,
      await call('POST', `/transactions/carts/${cartId}/approve`),
      {}
    );
  }
  record('carts list', 'GET', '/transactions/carts', await call('GET', '/transactions/carts'), {
    expectOk: true,
  });
  record(
    'reports/expiring',
    'GET',
    '/transactions/reports/expiring?window=30',
    await call('GET', '/transactions/reports/expiring?window=30'),
    { expectOk: true }
  );
  record(
    'reports/capacity',
    ...
    await call('GET', '/transactions/reports/capacity'),
    { expectOk: true }
  );
  record(
    'transaction log',
    ...
    await call('GET', '/transactions?limit=10'),
    { expectOk: true }
  );
  ...
  record(
    ...
    await call('POST', '/notifications/feedback', {
      body: { feedbackType: 'Bug', feedbackMessage: 'Title: e2e\n\nautomated functional test' },
    }),
    { expectOk: true }
  );
  ...
  if (editTarget) {
    record(
      'edit item',
      'PATCH',
      `/inventory/items/${editTarget}`,
      await call('PATCH', `/inventory/items/${editTarget}`, {
        body: { attributes: { ...attributes, notes: 'edited by e2e' } },
      }),
      { expectOk: true }
    );
    record(
      'item transactions',
      'GET',
      `/inventory/items/${editTarget}/transactions`,
      await call('GET', `/inventory/items/${editTarget}/transactions`),
      { expectOk: true }
    );
    record(
      'remove item',
      'POST',
      `/inventory/items/${editTarget}/remove`,
      await call('POST', `/inventory/items/${editTarget}/remove`, {
        body: { reason: 'incorrect_entry', note: 'e2e cleanup' },
      }),
      { expectOk: true }
    );
  }
  ...
  record(
    ...
    await call('POST', '/auth/account/password', {
      body: { currentPassword: password, newPassword: password + 'X' },
    }),
    { expectOk: true }
  );
